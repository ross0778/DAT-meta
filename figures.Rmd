...
# Save with explicit height/width — this is the most important fix for squishing
...
# Order predictors top-to-bottom
y1gpa_data$predictor <- factor(
  y1gpa_data$predictor,
  levels = rev(y1gpa_data$predictor)
)

LABEL_X <- 0.72  # fixed position for k/N labels

p <- ggplot(y1gpa_data, aes(x = r, y = predictor)) +
  ...
  labs(
    title = "Predictive Validity of DAT Subsections for GPA Year 1",
    y = NULL
  ) +
